...
use tars_transport::filter::Filters;
use tars_transport::transport::{ServerProtocolHandler, TarsServer, TarsServerConfig};
use tars_util::{ClientConfig, ServerConfig};
...
impl Application {
    ...
    pub fn use_client_filter_middleware(&self, middleware: tars_transport::filter::ClientFilterMiddleware) {
        self.filters.write().use_client_middleware(middleware);
    }

    /// Register server filter middleware
    pub fn use_server_filter_middleware(&self, middleware: tars_transport::filter::ServerFilterMiddleware) {
